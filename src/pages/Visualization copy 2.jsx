...
function Visualization() {
  ...
  const getChunkFilenames = (startDate, endDate) => {
    const filenames = [];
    let current = new Date(1999, 10, 1); // Start from 1999-11-01
    const lastValidStart = new Date(2024, 7, 1); // 2024-08-01
  
    while (current <= lastValidStart) {
      const startYear = current.getFullYear();
      const startMonth = current.getMonth() + 1; // JS months are 0-based
      const chunkStartDate = new Date(startYear, current.getMonth(), 1);
  
      // Calculate chunk end date (3 months later)
      const chunkEndDate = new Date(startYear, current.getMonth() + 3, 0); // 0 = last day of previous month
      const endYear = chunkEndDate.getFullYear();
      const endMonth = chunkEndDate.getMonth() + 1;
  
      // Check overlap
      if (chunkEndDate >= startDate && chunkStartDate <= endDate) {
        const filename = `/data/chunks_quarters/berkeley_${startYear}_${String(startMonth).padStart(2, '0')}_to_${endYear}_${String(endMonth).padStart(2, '0')}.csv`;
        filenames.push(filename);
      }
  
      // Move to next 3-month chunk
  ...
  const yearOptions = Array.from({ length: 2025 - 2000 }, (_, i) => 2000 + i);
